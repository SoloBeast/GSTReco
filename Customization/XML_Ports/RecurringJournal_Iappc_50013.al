xmlport 50013 "Recurring Journal Import"
{
    Caption = 'Recurring Journal Import';
    //Direction = Import;
    Format = VariableText;
    ...
    schema
    {
        textelement(VoucherImport)
        {
            ...
            tableelement("Gen. Journal Line"; "Gen. Journal Line")
            {
                ...
                textelement(RecurringMethod)
                {
                    MinOccurs = Zero;
                }
                textelement(RecurringFrequency)
                {
                    MinOccurs = Zero;
                }
                ...
                begin
                    ...
                    if (intCounter > 1) and (AccountType <> '') then begin
                        Evaluate(EnumRecurringMethod, RecurringMethod);
                        if EnumRecurringMethod = EnumRecurringMethod::" " then
                            Error('Recurring Method should not be blank');
                        ...
                        //Hemant
                        recGenLine.Validate("Recurring Method", EnumRecurringMethod);
                        Evaluate(varRecurringFrequency, RecurringFrequency);
                        recGenLine.Validate("Recurring Frequency", varRecurringFrequency);
    ...
    var
        ...
        EnumRecurringMethod: Enum "Gen. Journal Recurring Method";
        varRecurringFrequency: DateFormula;
}